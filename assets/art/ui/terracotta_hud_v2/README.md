...
- 坐标悬停读数只显示在右侧“位置”行，并由棋盘屏幕层统一换算镜头坐标；小地图通过独立预置 `BoardWorld` 鸟瞰视口复用主棋盘的 PlayerView、阵营镜像、区域、迷雾、墙、棋子与公开战术层。小地图单击/拖动使用可打断的 `0.16s` 二维镜头补间，缩放后灰色视野块可沿水平和垂直方向自由定位；滚轮保留 `0.32s` 平滑移动，棋盘不显示滚动条。
- 右侧战况列表按具体棋子聚合阵亡信息（例如 `兵*2, 士*1`）；棋子详情抽屉固定为左侧移动说明、右侧两个横排矩形操作按钮。
- `tools/ui/board_ui_layout_editor.html` 是 HUD 布局与文字调整工具；选择 HUD 后可用“编辑选中 UI 文字”直接调整内容、字体、对齐方式及文字框位置和尺寸，并继续使用原有 JSON 导入导出契约。网页画布已直接加载 `incense_assembly/` 的香盘、两柱香和信息展开栏，香盘预览保持完整龙虎轮廓并位于香体前景。
