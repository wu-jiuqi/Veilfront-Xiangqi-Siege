...
- 坐标悬停读数只显示在右侧“位置”行，并由棋盘屏幕层统一换算镜头坐标；小地图通过独立预置 `BoardWorld` 鸟瞰视口复用主棋盘的 PlayerView、阵营镜像、区域、迷雾、墙、棋子与公开战术层。小地图单击/拖动使用可打断的 `0.16s` 快速镜头补间，滚轮保留 `0.32s` 平滑滚动，灰色半透明区域实时显示主棋盘当前可见范围。
