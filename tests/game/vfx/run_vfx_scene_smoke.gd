...
func _run() -> void:
	...
	_expect(director.play_batch(peak_batch) == 9, "标准九族峰值应由预置池承载", failures)
	var peak_snapshot: Dictionary = director.get_pool_snapshot()
	_expect(peak_snapshot["active_count"] == 9, "九族峰值应保持九个活动效果", failures)
	_expect(peak_snapshot["overdraw_points"] == 100, "标准九族峰值预算应为100 points", failures)
	...
	_expect(director.play_batch(reduced_peak) == 9, "减少动态九族峰值应由预置池承载", failures)
	var reduced_peak_snapshot: Dictionary = director.get_pool_snapshot()
	_expect(reduced_peak_snapshot["overdraw_points"] == 37, "减少动态九族峰值应为37 points", failures)
	...
	var general_batch := CueContract.build_batch(
		"vfx-smoke-general", 11, 11, "standard", [general_cue]
	)
