...
func _capture() -> void:
	...
	var output_path := "res://evidence/ui/tutorial-codex-v2-p%02d-%dx%d.png" % [
		page_index,
		capture_size.x,
		capture_size.y,
	]
	...
	if error != OK:
		push_error("Could not save tutorial codex capture: %s" % error_string(error))
		quit(1)
		return
	print("TUTORIAL_CODEX_V2_CAPTURE_PASS path=%s" % ProjectSettings.globalize_path(output_path))
	quit(0)
