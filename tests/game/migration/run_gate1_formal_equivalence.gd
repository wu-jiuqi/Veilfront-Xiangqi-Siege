...
const GOLDEN_PATH: String = "res://tests/game/migration/golden/gate1-successor-20-seeds-v1.json"
const GOLDEN_SHA256: String = "e825641ca133eb9cae925d18e748660c7103fc5327120c5c48732027d6de0f4f"
...
func _run() -> void:
	...
	if not failures.is_empty():
		...
		quit(1)
		return
	print("FORMAL_EQUIVALENCE_PASS completed=%d replay_verified=%d channels=%d" % [
		seed_count, replay_verified, EXPECTED_CHANNELS.size(),
	])
	quit(0)
...
func _run_live_ranges(start_seed: int, seed_count: int, round_limit: int) -> Dictionary:
	...
	return {"ok": true, "completed": completed, "failure": ""}
...
func _load_golden_records(start_seed: int, seed_count: int, round_limit: int) -> Dictionary:
	...
	if str(document.get("schema_version", "")) != "veilfront-gate1-migration-golden-v1":
