...
func _run() -> void:
	...
	_assert((retry_button.get("art_texture") as Texture2D).resource_path.ends_with("failure_retry_button_v1.png"), "重试按钮没有使用独立抠图底板")
	_assert((back_button.get("art_texture") as Texture2D).resource_path.ends_with("failure_back_button_v1.png"), "返回按钮没有使用独立抠图底板")
	_assert((retry_button.get_node("%ArtLayer") as TextureRect).stretch_mode == TextureRect.STRETCH_KEEP_ASPECT_CENTERED, "失败操作底板必须保持比例")
