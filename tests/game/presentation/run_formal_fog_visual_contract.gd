...
func _run() -> void:
	...
	_expect(
		structure_layer.get_index() < piece_layer.get_index(),
		"public structures must remain below pieces"
	)
	_expect(
		intel_layer.get_index() < piece_layer.get_index(),
		"authorized intel must remain below pieces"
	)
