...
func _run() -> void:
	...
	if not _failures.is_empty():
		...
		quit(1)
		return
	print("AUDIO_RUNTIME_CONTRACT_PASS checks=%d cues=%d wav=34 pools=8+8" % [
...
func _check_catalog_and_sources() -> void:
	...
	_expect(stream_paths.size() == 34, "expected 34 active traceable WAV files")
	...
	_expect(CATALOG.definition_for("sfx.opening.gate_open") == null,
		"removed opening gate SFX must not remain in the active catalog")
