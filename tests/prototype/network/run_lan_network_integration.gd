...
func _run() -> void:
	...
	_check(str(client_session.get_session_role()) == "client" and str(client_session.get_local_seat()) == "black", "棋盘 UI 可查询客户端角色与黑方席位")
	_check(bool(server_session.can_submit_intents()), "红方先手时房主棋盘可提交动作")
	_check(not bool(client_session.can_submit_intents()), "红方先手时客户端棋盘不可提交动作")
	...
	if red_completed:
		_check(not bool(server_session.can_submit_intents()) and bool(client_session.can_submit_intents()), "动作推进后棋盘提交权切换到黑方")
