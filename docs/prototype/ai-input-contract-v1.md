...
入口不得增加完整棋局状态、规则随机流、场景树、规则核心节点、调试后门或任意状态查询回调。`AiPlayerView` 是技术负责人所拥有的单向玩家投影经 AI 白名单校验后的不可变副本；AI 层不实现、持有或反向查询完整状态。`AiPublicRules` 只含公开棋盘尺寸、公开棋子估值与公开行动类型偏置。`AiMemory` 只含该 AI 先前已知的行动 ID、行动访问次数、己方行动棋子的访问次数、历史公开敌棋观测与已知被吃敌棋 ID。

机器学习、在线服务以及读取迷雾内敌棋、隐身马、隐藏炮架、未公开炮击格、未公开撤回格或规则随机流，均不在接口可达范围内。

## 2. `AiPlayerView` v1 白名单

根字段只有：`schema_version`、`decision_id`、`viewer_side`、`turn_index`、`visible_cells`、`visible_pieces`、`public_flags`、`public_walls`、`legal_actions`、`public_events`。任何额外字段使输入失效，决策器返回 `invalid_ai_input`，不会尝试忽略或透传。

- 可见棋子：公开 ID、阵营、类型、坐标、公开状态标签。
- 旗帜：公开 ID、坐标、归属、占领方、公开占领进度与争夺状态。
- 城墙：阵营与公开状态。
- 公开事件：事件 ID、类型、行动阵营与公开坐标，不允许自由载荷。
- 合法动作：公开动作 ID、类型、行动棋子、起终点、可见吃子、可见侦察格数量、旗帜附近新增视野格数、是否占旗、是否攻击城墙、公开路径长度。
...
当前基线是人工编写的有界公开信息局面评价，并在公开信息上构造确定性信念假设：
...
4. 保帅权重略高于夺旗；直接攻击敌将与夺旗同级。夺旗接近、受威胁旗帜回防、占领进度和旗帜附近新增视野均有独立分项。
5. 第一层排序后，高分候选与关键候选进入有界二层搜索。二层只检查敌方最佳吃子、将威胁和旗帜压力，不复制规则核心。
6. 信念样本只使用公开标准初始阵型、历史公开观测、当前可见空格、已知阵亡记录和独立 AI seed；样本不得读取或校验 FullState 真值。
7. 对同一动作和同一行动棋子的累计访问分别施加可配置节奏惩罚；尚未投入的棋子首次行动具有一次性展开收益。吃子、占旗、解将等真实局面收益仍可覆盖节奏项，因而它不是棋种轮换配额。
8. 使用独立 AI 随机流加入可配置整数扰动；随机性只发生在策略候选已确定之后，同分时以公开动作 ID 稳定决胜。
9. 不用墙钟截止驱动候选数量。墙钟时间在不同机器上会改变动作，本原型只审计时间预算提示，实际硬边界使用候选数、敌方回应数和信念样本数。

规则随机种子和 AI 种子必须是不同随机流。建议技术集成层从“对局 AI 主种子 + 决策序号”派生本次 `ai_seed`，并把派生结果写入对局日志；不得把规则系统下一次随机结果或 RNG 对象传给 AI。

四档均使用完全相同的 `AiPlayerView` 白名单和 `visible-state-evaluation-v1`。档位差异来自候选上限、二层候选数、敌方回应上限、信念样本数、风险聚合、最终随机扰动和可配置权重；专家档覆盖更宽候选且随机扰动为零。信念棋子仅是假设样本，不是不可见真值；因此该评价是公平的公开信息策略增强，不声称拥有完整信息极小化搜索。
