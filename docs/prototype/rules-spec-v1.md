...
状态：`prototype / iteration 2 / owner-freeze revision 4`。本文件规范项目简报 `confirmed` 语义及项目所有者后续冻结结论。实现架构仍为 `hypothesis`，剩余 `unknown` 不得被实现默认值冒充批准规则。
...
- 项目所有者将两条城墙线向各自大本营移动两行，使兵/卒初始格位于城墙线上；旗帜改为完整战区随机且永久隐藏坐标；相视野扩为起点中心3x3、路径田字格和终点中心3x3并集，并仅阻挡敌车首次进入；士改为主动献祭复活，阵亡士与帅/将不进入随机池。

## 1. 坐标、区域与回合术语

- 服务端固定使用红方视角坐标：`X=1..9, Y=1..24`，共 216 格；红方向 `+Y` 前进，黑方向 `-Y` 前进。客户端镜头可旋转，但不得改变底层坐标。
- 红方大本营 `Y=1..3`、红方缓冲区 `Y=4..8`、中央战区 `Y=9..16`、黑方缓冲区 `Y=17..21`、黑方大本营 `Y=22..24`。红墙位于 `Y=3/4` 间，黑墙位于 `Y=21/22` 间；兵/卒初始格分别在 `Y=4`、`Y=21` 的城墙线上，特殊行动区域为 `Y=4..21`。`Y=12/13` 只可称为棋盘几何中线，不恢复河界或过河规则。
...
- 各车路径视野源、相视野源与敌车阻挡源、随机种子/消费序号，以及按阵营 FIFO 的后备部署队列；
- `BombardmentResult { random_seed_ref, target_center, impact_cells[3], impact_order[3], simultaneous_resolution_id }`。

必须保持以下不变量：一格最多一枚棋子；`DEAD/RESERVE` 棋子不在棋盘；后备棋子不可行动、被攻击、提供视野、充当炮架/阻挡或计入墙/旗；炮区域弹药每门初始 2、只减不增；FullState 不得存在阵营共享轰炸冷却字段、计时或重置；士复活动作必定先牺牲发动者，阵亡士与帅/将不得进入复活池；双方城墙独立；终局后不再接受行动。
...
| 相/象 | 合资格移动无视堵象眼；视野为起点中心 `3x3`、起终点组成的田字格九点、终点中心 `3x3` 的并集，投影时裁剪到棋盘。田字格九点形成敌车阻挡源：敌车从该田字格外试图进入或穿过时，只移动到路径首个交点；下一次从田字格内离开时恢复正常。该源不阻挡己方车。 |
...
- 开局从完整中央战区 `X=1..9,Y=9..16` 抽取三个不同格；旗帜坐标始终不进入 PlayerView、AI DTO 或 LAN 下行，规则种子也不得下发到可据此反推旗位的客户端。
- 棋子行动后停在中立旗或敌方所有旗上时，以该 `piece_id` 开始 `1/3` 占领，并只公开“红方/黑方正在夺旗(n/3)”进度；完成后只公开“红方/黑方成功夺得一面旗帜”，不公开具体旗位。之后每当对方完成一次行动机会（含三类跳过），若该棋子仍在旗格，进度加 1。
