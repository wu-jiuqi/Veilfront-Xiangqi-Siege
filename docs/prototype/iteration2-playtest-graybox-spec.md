...
- 项目所有者本轮新增要求：最终人工试玩版提供简单/中等/困难三档公平 AI，并把本轮试玩版随机回归调整为 100 个固定种子。本要求不改写 Contract v2、QA-P1-003、Registry 或既有批准记录。
...
- “具可观察差异”的最低证据是：三份配置摘要各不相同；在候选数足以触发预算差异的同一公开测试场景中，实际评估候选数符合 8/32/96 上限；100 个固定种子报告三档的动作分布、胜负/胜因、局长和决策审计差异。不得通过泄露隐藏状态或为困难档增加真值查询来制造差异。
...
受控本地记录至少包含：构建/提交标识、规则/实现 revision、种子、人类阵营、`full_round_limit_hypothesis=50` 或实际 CLI 覆盖值、AI 试玩标签、profile digest、候选预算、随机扰动范围、AI seed 引用、规范化 intent 序列、公开事件摘要、最终 winner/win_reason、完整轮数、重放摘要。完整审计与玩家可见记录分开保存；玩家界面和截图只能访问后者。
...
- [ ] 三档配置摘要不同，候选预算/随机扰动与 8/±12、32/±4、96/±1 的 hypothesis profile 一致，并有固定场景和批次统计证明差异可观察。
...
- [ ] Iteration 2 试玩版执行 50 回合配置下的同一组 100 个固定种子，并分别跑简单/中等/困难三档（共 300 个 profile-run）：每档都要形成公平性/差异证据、合法终止和确定性复跑，失败数与确定性差异均为 0。固定种子门槛口径仍为 100，不能把 300 个 profile-run 冒充 300 个或 1000 个不同固定种子；该结果不满足 Contract v2 门槛。

### 状态转换限制

- 当前 Contract v2 的 `CHECK-SIMULATION-001` 要求至少 1000 个固定种子；QA-P1-003 临时兼容条件也明确要求 1000/1000、失败数 0、确定性差异 0。
- 因此即使本规格所有灰盒检查、三档 AI 检查、独立 QA 和 100/100 固定种子都通过，本轮状态仍为 `active`，结论只能写“Iteration 2 试玩版回归完成，Contract 门槛未满足”。
...
- 灰盒、三档 AI、独立 QA 与 100/100 回归通过后仍保持 `active`。只有补足当前 Contract v2 的 1000/1000，或完成项目所有者另行批准的 Contract 修订及迁移后，才可考虑 `active -> review`；任何路径都不能自动批准 GATE-1 或开始正式生产。
