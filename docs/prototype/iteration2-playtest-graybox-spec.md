...
- 项目所有者本轮新增要求：最终人工试玩版提供简单/中等/困难三档公平 AI，并把本轮试玩版随机回归调整为 100 个固定种子。本轮自动证据分为同一 seed 集合上的“三档 AI 单决策矩阵”和“规则压力完整对局”，两者互补但不得互相冒充。本要求不改写 Contract v2、QA-P1-003、Registry 或既有批准记录。
...
灰盒必须让一名人类玩家选择简单/中等/困难三档之一，与对应的 PlayerView 公平 AI 从固定种子开局，经过选择、预览、确认、结算、AI 回合和反馈，完整运行到三旗、将帅实际死亡或第 50 个完整轮边界的轮上限结算。试玩的目标是判断规则逻辑、信息反馈、AI 体验差异和核心循环是否合理，不评价正式视觉品质。

一次可验收试玩至少满足：

1. 可输入或沿用一个整数种子开始新局，并显示本局种子、代码版本和 `50（可覆盖假设）`。
2. 玩家阵营在开局时确定；红方始终先手，同一种子与配置必须得到同一阵营分配、旗位和后续确定性结果。
3. 人类能在开局前选择简单/中等/困难，三档均能仅凭各自 PlayerView 接续完成行动，并产生可审计、可体验的差异。
...
- 难度不得扩大信息权限。简单/中等/困难在相同 `AiPlayerView`、公开规则和记忆下获得相同公开候选全集；档位只改变批准的候选预算与随机扰动等 hypothesis 参数。隐藏等价判据按“同一难度、同一 AI 种子”分别成立，不要求不同难度彼此选择相同动作。
...
- 最终人工试玩版必须提供三个开局前可选、局内锁定的 hypothesis profile：

| 试玩标签 | `profile_id` | 候选上限 | 随机分幅度 | 公开评分权重（侦察/旗/墙/重访惩罚） | 预期可观察差异 |
|---|---|---:|---:|---|---|
| 简单 | `prototype-low-budget-hypothesis` | 8 | ±12 | 2 / 16 / 6 / 1 | 评估候选更少、动作扰动更明显 |
| 中等 | `prototype-default-hypothesis` | 32 | ±4 | 3 / 20 / 8 / 2 | 候选覆盖与扰动居中 |
| 困难 | `prototype-high-budget-hypothesis` | 96 | ±1 | 4 / 24 / 10 / 3 | 评估候选更多、动作更接近公开规则评分排序 |

  以上标签只服务本轮人工试玩，不冻结正式难度曲线，也不保证胜率严格单调。时间预算仍只是审计提示，不能用墙钟截止改变确定性结果。
- “具可观察差异”的自动证据来自第 9.4 节 A 矩阵：三份配置摘要各不相同；在候选数足以触发预算差异的同一公开输入中，实际评估候选数符合 8/32/96 上限，并报告三档单次决策的候选抽样、评分与最终动作差异。A 矩阵不是完整对局，不报告也不推断胜负、胜因或局长。不得通过泄露隐藏状态或为困难档增加真值查询来制造差异。
...
人工试玩负责形成完整人类对阵简单/中等/困难三档 AI 的体验反馈。每档至少应完成可形成有效反馈的一局，但本轮不新增“三档各 100 局完整对局”之类的自动数量门槛；人工结果不得用来替代 A/B 自动证据。
...
7. 分别记录简单/中等/困难的公平感、挑战感、可感知差异及怀疑作弊时的具体可见证据；
...
两条证据使用同一组 100 个固定种子，但测试对象、记录单位和允许结论不同。它们不得相加为 200 seeds，也不得把 A 的 300 records 当作 300 局完整对局。

#### A. PlayerView AI 单决策公平/差异矩阵

- 对同一组 100 seeds，分别运行简单/中等/困难三个 profile，形成 `100 × 3 = 300` 条 seed-profile 单决策 records。
- 每条 record 使用规范化的公开 `AiPlayerView`、公开规则、AI 记忆、对应 profile 和派生 AI seed；隐藏等价配对可封装在同一条 record 中，验证同 profile、同输入、同 AI seed 时输入摘要、候选抽样、评分记录和最终动作完全一致。
- 跨 profile 比较只用于证明候选预算、随机扰动、公开评分权重和最终动作存在可审计差异；不同档位不要求选择相同动作。
- A 是单决策矩阵，不推进完整对局，不要求合法终局，也不报告胜负、胜因、完整轮数或局长。它只支持三档 AI 的 PlayerView 公平性、确定性与参数差异结论。
...
- `rules_stress_full_state_policy` 明确可读取 FullState 真值，只用于规则状态机、结算、终止、确定性和回放压力验证；它不是简单/中等/困难任一 PlayerView AI，不得作为三档 AI 公平性、强度、胜率或体验差异证据。

#### 两条证据与人工试玩的关系

- A 回答“三档是否只看等价 PlayerView，且参数/单次决策差异是否可审计”。
- B 回答“规则核心在 100 个固定种子、50 回合假设下能否完整、确定、可重放地终止”。
- 人工试玩回答“完整人类 vs 三档 AI 是否形成可感知、公平且合理的体验”。人工试玩、A、B 互相补充，任何一条都不得代替另外两条。
...
- [ ] 简单/中等/困难三档均可从开局选取并完成一局；局内不可切换，三档使用同一 PlayerView 白名单和公开候选权限。
- [ ] 三档配置摘要不同，候选预算/随机扰动与 8/±12、32/±4、96/±1 的 hypothesis profile 一致，并由 A 单决策矩阵证明 PlayerView 公平性及差异可观察。
...
- [ ] 人工试玩分别对简单/中等/困难形成完整人类对局与体验反馈，并绑定种子/版本；不要求三档各 100 局完整对局。
- [ ] A：同 100 seeds × 三档形成 300 条 PlayerView AI 单决策 records，完成隐藏等价公平性、同配置确定性及跨配置差异检查；不写胜负、胜因或局长。
- [ ] B：同 100 seeds 各完成一局 50 回合上限 `rules_stress_full_state_policy` 完整对局，100/100 合法终止，失败数 0、确定性差异 0，并报告胜负、胜因、局长和重放核对；不写成三档 AI 公平或强度证据。
- [ ] A 的 300 records 不是 300 场完整对局，B 的 100 场也不是三档 AI 对局；A、B 共用的固定 seed 门槛口径仍为 100，不能相加或冒充 1000。该结果不满足 Contract v2 门槛。

### 状态转换限制

- 当前 Contract v2 的 `CHECK-SIMULATION-001` 要求至少 1000 个固定种子；QA-P1-003 临时兼容条件也明确要求 1000/1000、失败数 0、确定性差异 0。
- 因此即使本规格所有灰盒检查、人工三档试玩、A 的 300 条单决策 records、B 的 100/100 规则压力完整对局和独立 QA 都通过，本轮状态仍为 `active`，结论只能写“Iteration 2 试玩版回归完成，Contract 门槛未满足”。
...
- 灰盒、人工三档试玩、A 单决策矩阵、B 的 100/100 规则压力完整对局和独立 QA 通过后仍保持 `active`。只有补足当前 Contract v2 的 1000/1000，或完成项目所有者另行批准的 Contract 修订及迁移后，才可考虑 `active -> review`；任何路径都不能自动批准 GATE-1 或开始正式生产。
