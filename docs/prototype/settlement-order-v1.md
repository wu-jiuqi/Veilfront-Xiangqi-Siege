...
状态：`prototype / iteration 1 / owner-freeze revision 3`。本表是 `rules-spec-v1.md` 的唯一结算顺序，不扩展规则语义。
...
| 3 | 意图解析/位移 | 某相/象的下一次移动开始时，先清除该棋旧田字显形区。随后解析意图：隐藏阻挡通常导致棋子不动但消耗行动，并只发布统一模糊接触结果；普通车中途首次接触未显形敌方隐身马是例外，改为在该交点完成吃子并停止。成功则执行位移。特殊车按路径顺序逐目标；兵/卒特殊穿越不伤害。 |
...
- **车路径**：目标按从起点到终点排序；对每个目标完整执行“阵亡候选→实际死亡→将帅死亡检查”，才可处理下一个目标。普通车中途首次接触未显形敌方隐身马时，将该马作为接触目标结算并把车的终点截断到该交点；特殊车继续按既有穿越规则处理。
- **相/象显形**：移动开始先删该棋旧源；仅合资格成功移动建立新源。新源严格包含起点—终点包围方形九格（含起点、象眼、终点）；多个源取并集，删除一个源不得删除其他源仍覆盖的格。
