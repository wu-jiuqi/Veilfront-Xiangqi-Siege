# 局域网真人测试工具 Godot 产物映射 v1

状态：`approved disposable tooling / pre-integration`
Contract：`LOOP-CTR-GATE1-VERTICAL-SLICE-001@v3`
...
| 产物 | Godot 类型 | 路径 | 责任 |
|---|---|---|---|
| 协议白名单与消息校验 | `RefCounted` GDScript | `scripts/prototype/network/lan_protocol.gd` | 规范化 intent、限制请求字段、检测下行受禁字段 |
...
- 服务器下行只允许 `schema_version`、`protocol_version`、`seat`、`player_view` 与公开反馈。
- 客户端不得接收 `board`、`rng`、`prepared_action`、`events`、`vision_sources`、`state_digest`、`event_log_digest` 或完整 FullState。
- 服务端只接受规范化的 `{piece_id, action_type, target_cell, skill_type}`，并额外校验席位、当前行动方、`action_index`、请求 ID 与公开候选。

## 首版固定假设

- 房主固定红方，加入者固定黑方，红方仍按既有规则先手。
- 只允许一个远端客户端；不自动发现、不重连、不迁移房主。
...
4. 每次下行递归检查受禁字段；客户端快照不包含 FullState、RNG 内部状态或完整审计。
