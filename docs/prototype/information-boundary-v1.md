...
状态：`prototype / iteration 2 / owner-freeze revision 4`。目标是让 UI、AI 与 LAN 客户端只能看到玩家等价信息，并支持付费于一次行动的有限接触侦察。

冻结来源：`OWNER-FREEZE-2026-08-15` = `C:/Users/30114/.codex/attachments/dd52e25b-1af0-4fb5-90d1-a5315853a81c/pasted-text.txt`。

## 变更摘要

行动提示冻结为 `KNOWN_LEGAL / TENTATIVE / KNOWN_ILLEGAL`，玩家提交行动意图；隐藏路径、腿眼、目标棋、炮架和敌方相田字阻挡的解析与最小披露已有唯一规则。旗帜位置与可反推旗位的规则种子永久留在主机 FullState/审计层；PlayerView、AI DTO 与 LAN 下行只携带旗帜所有权和占领进度。
...
- 公开：棋盘尺寸/区域、当前行动方、完整轮计数、双方城墙公开状态、三旗所有权与已公开占领进度、终局结果；不包含旗位或规则种子。
...
相/象视野源的投影定义为：对本次合资格合法移动起点 `O` 和终点 `D`，输出 `center3x3(O) ∪ field3x3(O,D) ∪ center3x3(D)`，投影时裁剪棋盘边界。敌方只能从车实际停在首个交点这一公开结果获知本次路径受阻，不得在预览或 DTO 中获得敌方田字格源。该棋下次移动开始或离场、死亡、回营、入队时旧源消失；敌墙倒塌移除能力时亦消失。
...
迷雾内敌棋及类型/位置/资源；隐身马；旗帜位置与可反推旗位的规则种子；未被当前投影授权公开的炮架；未公开随机结果（炮击抽格、主动士复活选择、复活/撤回落点在其公开前）；RNG 状态和未来抽样；敌方不可见相视野/阻挡区/路径源的内部标识；完整合法行动真值、碰撞缓存、评估分、状态摘要和调试日志。
