...
- 主棋盘使用等距正方形交点网格与点状行动高亮；红黑客户端均将己方大本营旋转到屏幕底部。区域只以亮色底块和区域内半透明大字区分，普通区域分割线不进入表现层；不可见格使用黑色半透明蒙版，可见格穿透蒙版。己方特殊视野中，车只沿完整移动路径绘制蓝色网格线，相/象只沿田字阻挡九点的外沿绘制黄色网格线；扩展侦察并集仍参与视野计算但不单独描边。
- 玩家未选中棋子时可右键添加圆/叉/方形标注；选中棋子后的第一次右键只取消选择，下一次右键才打开标注。标注只存在本机表现层，不写入 PlayerView、不联网同步，也不得覆盖行动高亮。
