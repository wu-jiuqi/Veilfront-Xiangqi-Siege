...
| VS-WALL-01 | 红方平面城防阵线完整/倒塌/修复中样片 | 3状态 | 静态源图与预置场景已完成；待Godot运行时与独立QA |
| VS-PIECE-01 | 红方步兵透明立绘 | 1 | 1024×1536设计源已确认；内置生成器Alpha失败，未进入运行时目录 |
| VS-PIECE-02 | 黑方步兵透明立绘 | 1 | 1024×1536设计源已确认；待真实Alpha，未进入运行时目录 |
| VS-PIECE-03 | 红方骑兵透明立绘 | 1 | 1254×1254单体概念V1已生成；待所有者审查、真实Alpha与源尺寸补齐 |
| VS-PIECE-04 | 黑方骑兵透明立绘 | 1 | 1254×1254单体概念V1已生成；待所有者审查、真实Alpha与源尺寸补齐 |
| TW-PIECE-SILHOUETTES | 红黑14棋子轮廓审查页 | 1 | V2已获所有者确认；只批准继续Contract内最小样片，非最终资产 |
...
- Y=4放置完整红方平面城防阵线，五枚兵仍在同一交点线上；黑方平面变体通过红方样片审查后再制作。观察贴地层级、状态可读性与点击。
