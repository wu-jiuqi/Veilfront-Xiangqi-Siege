...
- UI 基础合约：`UI_COMPLETE_REBUILD_FOUNDATION_PASS`。
- 静态主题合约：`UI_STATIC_THEME_CONTRACT_PASS roots=19 production_scenes=14 theme=v2`。
...
- 前台主流程：主菜单、设置、关卡选择、加载和正式局域网大厅通过。
...
- 后续新增正式页面必须复用 V2 Theme、按钮预置场景和现有表面角色；不得重新引入以完整 PNG 决定布局的按钮或面板。
