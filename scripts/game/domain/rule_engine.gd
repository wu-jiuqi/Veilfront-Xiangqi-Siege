...
static func list_legal_actions(state: Dictionary, side: String = "") -> Array:
	...
	return MoveRules.generate_legal_actions(
		state, actor_side, VisibilityPolicy.context(state, actor_side)
	)
	...
	match action_type:
		...
		"move":
			outcome = _resolve_move(
				...
				intent,
				actor_side,
				bool(options.get("trusted_generated_action", false)),
				str(options.get("public_classification", "TENTATIVE"))
			)
		"bombard":
...
static func _resolve_move(
	...
	public_classification: String = "TENTATIVE"
) -> Dictionary:
	var visibility_context: Dictionary = VisibilityPolicy.context(state, actor_side)
	if not trusted_generated_action and public_classification == "KNOWN_ILLEGAL":
...
static func _update_flag_discoveries(state: Dictionary) -> void:
	for side: String in [MatchState.RED, MatchState.BLACK]:
		...
		for flag: Dictionary in state["flags"]:
			...
			if VisibilityPolicy.is_cell_visible(state, side, cell):
...
static func _publish_player_event(state: Dictionary, actor_side: String, action_index: int, outcome: Dictionary) -> void:
	...
	if MatchState.is_inside_board(position):
		public_to_opponent = public_to_opponent or VisibilityPolicy.is_cell_visible(
...
static func _authorized_captures(
	...
	for casualty: Dictionary in casualty_records:
		...
		if not allowed and MatchState.is_inside_board(casualty_cell):
			allowed = VisibilityPolicy.is_cell_visible(state, viewer_side, casualty_cell)
		if not allowed:
			continue
		seen[piece_id] = true
		captures.append({
