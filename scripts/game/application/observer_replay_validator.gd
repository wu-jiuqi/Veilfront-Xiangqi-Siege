...
const ROOT_FIELDS: Array[String] = [
	...
	"frames", "final_player_view_digest", "codec_versions",
]
const FRAME_FIELDS: Array[String] = [
	"action_index", "player_view_or_digest", "visible_events", "visible_error",
	"action_previews",
]
...
static func validate(record: Dictionary, bound_side: String) -> Dictionary:
	...
	or str(record.get("schema_version", "")) != "veilfront-observer-replay-v1" \
