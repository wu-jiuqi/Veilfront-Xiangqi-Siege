...
const ELEPHANT_REVEAL_COLOR := Color(1.0, 0.78, 0.18, 0.9)
const ELEPHANT_BLOCK_COLOR := Color(0.2, 0.9, 0.94, 0.9)
...
func _draw() -> void:
	...
	for record: Dictionary in _overlays.get("elephant_reveal_zones", []):
		_draw_field_outline(record.get("cells", []), ELEPHANT_REVEAL_COLOR, 12.0)
