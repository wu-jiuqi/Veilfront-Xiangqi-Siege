...
func submit_intent(intent: Dictionary) -> Dictionary:
	...
	if not is_local_turn():
		return {"ok": false, "error": "not_local_turn"}
	var normalized_intent: Dictionary = LanProtocol.normalize_intent(intent)
...
func is_local_turn() -> bool:
	return not _seat.is_empty() \
		and not _current_player_view.is_empty() \
		and not bool(_current_player_view.get("terminal", false)) \
		and str(_current_player_view.get("active_side", "")) == _seat


func can_submit_intents() -> bool:
	return is_local_turn() and _connection_state not in [
		"connection_error",
		"connection_failed",
		"disconnected",
		"join_rejected",
...
func preview_intent(intent: Dictionary) -> Dictionary:
	...
	if normalized_intent.is_empty():
		return {}
	return PlayerViewProjector.preview_intent(
		_current_player_view,
		normalized_intent
	).duplicate(true)
